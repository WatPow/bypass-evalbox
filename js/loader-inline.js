/**
 * EvalBox Bypass Loader (version inline)
 * Script pour générer le code d'injection à copier dans la console
 */

(function() {
  // ID unique pour cette session
  const SESSION_ID = 'bypass_' + Math.random().toString(36).substring(2, 15);
  ...
  // Générer le code d'injection complet (intégré directement)
  function generateInjectionCode() {
    // Le contenu du script est directement intégré ici
    return `
...
class EvalBoxBypass {
  ...
  neutralizeUserAgentDetection() {
    // Stocker une valeur fixe
    const userAgentValue = navigator.userAgent;
    
    // Écraser uniquement si ce n'est pas déjà fait
    if (Object.getOwnPropertyDescriptor(Navigator.prototype, 'userAgent') && 
        Object.getOwnPropertyDescriptor(Navigator.prototype, 'userAgent').configurable) {
      Object.defineProperty(Navigator.prototype, 'userAgent', {
        get: function() { return userAgentValue; }
      });
    }
  }
  ...
  neutralizeDevToolsDetection() {
    // Version sécurisée qui évite la récursion infinie
    const consoleProps = ['profiles', 'memory', 'timers'];
    
    // Neutraliser des propriétés spécifiques sans toucher à l'objet console entier
    consoleProps.forEach(prop => {
      if (console[prop]) {
        // Remplacer par des valeurs vides ou factices plutôt que de redéfinir tout l'objet
        console[prop] = typeof console[prop] === 'function' ? function(){} : [];
      }
    });
    
    // Neutraliser la détection via les temps d'exécution en sauvegardant une référence directe
...
// Initialiser le bypass
(function() {
  console.log("[EvalBox Bypass] Démarrage du système avancé...");
  
  // Créer et initialiser l'instance
  const bypass = new EvalBoxBypass();
  bypass.initialize();
  
  // Exposer l'instance globalement pour le debugging si nécessaire
  window.__bypassInstance = bypass;
  
  // Notifier l'utilisateur
  console.log("[EvalBox Bypass] Protection complète activée!");
  alert("Protection avancée contre EvalBox activée avec succès. Vous pouvez maintenant naviguer librement.");
})();`;
  }
  ...
  function init() {
    if (generateBtn) {
      generateBtn.addEventListener('click', function() {
        log("Génération du code d'injection...");
    ...
    log("Page chargée. Prêt à générer le code d'injection avancé.");
  }
